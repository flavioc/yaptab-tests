...
btree_nodes(Nodes) :-
  Depth is truncate(log10(Nodes)/log10(2)),
  btree(Depth).
...
chain(Depth) :-
  atomic_concat(['data_edge_chain_',Depth,'.pl'],File),
  atomic_concat(['Chain configuration with depth ',Depth],Program),
  open(File,'write',FO),
  tell(FO),
  write_header(File,Program),
  edge_chain(Depth, 1),
...
edge_chain(Depth, Depth) :- !.
edge_chain(Depth, Cur) :-
  Next is Cur + 1,
  write_edge(Cur, Next),
  edge_chain(Depth, Next).
